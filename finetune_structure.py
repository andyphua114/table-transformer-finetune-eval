...
def train():
    ...
    train_dataset = CocoDetection(
        img_folder=r"C:\Users\andyp\projects\label-studio\data-table-images",
        processor=processor,
        train=True,
    )
    ...
    eval_compute_metrics_fn = partial(
        compute_metrics, image_processor=processor, id2label=id2label, threshold=0.05
    )

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    class WeightedTableTransformer(TableTransformerForObjectDetection):
        ...
        def forward(self, **inputs):
            ...
            if "loss" in outputs:
                ...
                # Replace classification loss with weighted version
                class_logits = loss_dict[
                    "logits"
                ]  # logits shape: [batch_size, num_queries, num_classes]
                labels = loss_dict["labels"]  # labels shape: [batch_size, num_queries]

                # Flatten
                logits_flat = class_logits.view(-1, class_logits.size(-1))
                ...
                # Replace the original classification loss
                outputs["loss"] = weighted_loss + loss_dict["bbox"] + loss_dict["giou"]
    ...
    training_args = TrainingArguments(
        output_dir="tt_structure_finetuned",
        num_train_epochs=1,
        fp16=False,
        ...
        report_to="none",
        logging_dir="./logs",
        logging_strategy="epoch",  # Optional but explicit
    )
